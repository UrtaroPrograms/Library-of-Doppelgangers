...
func _update(_delta: float):
	...
	if(character.velocity.y > 0):	#Si la velocidad en Y es mayor a 0, estamos bajando. Flippeamos el vertical. Si hubiese animaciones acá llamaríamos la de caminar hacia abajo.
		character._call_animation("caminar")  #Si tuviesesmos animaciones para arriba, abajo, izquierda y derecha, acá llamaríamos la de "abajo"
		#character._flip_vertical_appearance(true)
		character.moveLantern(0)
		character._turnRaycast(0)
		...
		character._call_animation("caminar")
		character._flip_vertical_appearance(false)
		character._turnRaycast(180)
		character.moveLantern(2)
